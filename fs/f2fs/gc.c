		...
		struct f2fs_gc_kthread *gc_th, bool val)
{
	if (val) {
		if (!wake_lock_active(&gc_th->gc_wakelock)) {
			f2fs_err(sbi->sb, KERN_INFO, "Catching wakelock for GC");
			wake_lock(&gc_th->gc_wakelock);
		}
	} else {
		if (wake_lock_active(&gc_th->gc_wakelock)) {
			f2fs_err(sbi->sb, KERN_INFO, "Unlocking wakelock for GC");
...
do_gc:
		...
		if (f2fs_gc(sbi, force_gc || test_opt(sbi, FORCE_FG_GC), true, NULL_SEGNO)) {
			 ...
			 */
			mm_drop_caches(3);
			f2fs_err(sbi->sb, KERN_INFO, "dropped caches");
		}
